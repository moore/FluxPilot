...
`RET` uses the current `frame_pointer` to locate the saved `return_pc` and
saved frame pointer, removes them from the stack, restores `frame_pointer`,
and sets the program counter to `return_pc`.
...
- `RET`: restore frame pointer, jump to saved return PC (does not unwind the
  host call to `run`).
...
- `EXIT` is a simple return that does not unwind the call frame. Use `RET`
